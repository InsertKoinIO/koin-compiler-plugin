...
class DslHintGenerator(
    ...
    /**
     * Relay DSL definition hints for every LOCAL module's `includes()` edge that points at a module
     * NOT declared in this compilation — i.e. a dependency module, only reachable through the edge
     * rather than this compilation's own classpath. See [relayDslDefinitionHints] for why this is
     * needed. A no-op when every included module is local (its own hints are already generated by
    ...
    fun relayIncludedDefinitionHints(
        ...
        val farIncludedIds = moduleIncludes.values.flatten().filterTo(mutableSetOf()) { it !in localModuleIds }
