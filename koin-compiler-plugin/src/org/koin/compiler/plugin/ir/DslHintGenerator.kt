...
class DslHintGenerator(
    ...
    fun generateDslDefinitionHints(
        ...
        for (groupKey in groupKeys) {
            ...
            disambiguateDuplicateSignatures(functions)
    ...
    /**
     * Disambiguate JVM-signature collisions among hint functions about to land in the SAME file.
     *
     * Two unrelated definitions of the SAME type, with the SAME bindings/requirements/qualifier
     * shape (most commonly two provider-only anonymous `single<T> { ... }` blocks in one file — no
     * `val`/`fun` id, no constructor params to distinguish them), erase to an IDENTICAL JVM method
     * descriptor: parameter NAMES (`req0_x` vs `req0_y`, or here, nothing at all) don't affect
     * erasure, only parameter TYPES do, and Unit-typed markers all erase to the same `Lkotlin/Unit;`
     * regardless of what they're named. Two hint functions sharing (name, erased param types) in one
     * class file is an invalid classfile — `Duplicate method name "dsl_single" with signature
     ...
     * functions (one distinct shape per file) keep their existing signature unchanged.
     */
    private fun disambiguateDuplicateSignatures(functions: MutableList<IrSimpleFunction>) {
        val seen = mutableMapOf<String, Int>()
        for (i in functions.indices) {
            val function = functions[i]
            val key = function.name.asString() + "(" + function.parameters.joinToString(",") { it.type.render() } + ")"
    ...
    fun discoverDslDefinitionsFromHints(): List<Definition.DslDef> {
        ...
        // Dedupe: [relayIncludedDefinitionHints] re-publishes a far module's own hints under THIS
        // compilation's output so a reader one hop further away can see them — but when the far
        // module is ALSO directly classpath-visible (e.g. it's a direct Gradle dependency, not
        // genuinely 2+ hops from here), both the original hint AND the relay resolve in this same
        // scan, surfacing the identical definition twice and double-reporting every diagnostic
        // against it. Keyed on (module, type, definition kind, qualifier, bindings, providerOnly) —
        // an exact match on all of these can only be the same source definition seen twice, never two
        // distinct real definitions.
        val deduped = definitions.distinctBy { def ->
            val bindingsKey = def.bindings.mapNotNull { it.fqNameWhenAvailable?.asString() }.sorted()
            listOf(
                def.modulePropertyId ?: "",
                def.irClass.fqNameWhenAvailable?.asString() ?: "",
                def.definitionType,
                def.providerOnly,
                def.qualifier,
                bindingsKey,
        ...
        if (deduped.isNotEmpty()) {
            KoinPluginLogger.debug { "  Discovered ${deduped.size} DSL definitions from dependency hints" }
        }

        return deduped
    }

    /** Compilation-wide dedupe for [relayDslDefinitionHints] — see its kdoc. */
    private val relayedDslModuleIds = mutableSetOf<String>()

    /**
     * Relay a DEPENDENCY module's own `dsl_single`/`dsl_factory`/etc. hints — visible only via THIS
     * compilation's OWN classpath — under [includedModuleId], so a reader one hop further away (whose
     * classpath doesn't reach [includedModuleId] directly) can still discover them.
     *
     * `module { includes(a) }` crosses the implementation-hop boundary via the `dslincludes_*`
     * topology carrier (see [discoverModuleIncludesFromHints]) — that carrier only needs to know the
     * EDGE, so it survives even when `a` itself is classpath-invisible. But `a`'s own DSL definition
     * hints are ordinary declarations compiled into `a`'s OWN output: a reader 2+ implementation hops
     * from `a` never has it on its classpath, so [discoverDslDefinitionsFromHints] finds nothing there
     * regardless of how many `includes()` edges the walk crosses. Same root cause and fix shape as
     * `KoinAnnotationProcessor.relayIncludedModuleHints` / `relayConfigurationHints`: an intermediate
     * module that CAN see `a` directly (it's on ITS classpath) re-publishes `a`'s hints under its own
     * compiled output.
     *
     * Deduped compilation-wide by [includedModuleId] — two local modules both including the same far
     * module only relay it once. Returns the built hint functions unparented; the caller batches them
     * into one file (see [relayIncludedDefinitionHints]).
     */
    private fun relayDslDefinitionHints(includedModuleId: String): List<IrSimpleFunction> {
        if (!relayedDslModuleIds.add(includedModuleId)) return emptyList()
        val defs = discoverDslDefinitionsFromHints().filter { it.modulePropertyId == includedModuleId }
        val hints = defs.mapNotNull { buildDslHintFunction(it) }
        if (hints.isNotEmpty()) {
            KoinPluginLogger.debug {
                "  + relayed ${hints.size} DSL definition hint(s) from classpath-invisible include $includedModuleId"
            }
        }
        return hints
    }
    ...
    /**
     * Relay DSL definition hints for every module — LOCAL or not — reachable from a local `includes()`
     * edge through a chain of non-local (dependency) modules. See [relayDslDefinitionHints] for why a
     * single non-local module needs relaying at all.
     *
     * Walks the FULL transitive closure, not just the immediate edge: a pure aggregator module (e.g.
     * `fun repositoryModule() = module { includes(a); includes(b) }`, zero definitions of its own) is
     * itself a genuine non-local include, so relaying only ITS OWN (empty) defs is a no-op — the real
     * providers sit one level further inside `a`/`b`, invisible to a one-hop relay. Real-world example
     * that motivated this: `batchKoinModule() -> repositoryModule() -> sharedRepositoriesModule()`,
     * three Gradle modules, the middle one a bare relay with no definitions of its own. Mirrors
     * [org.koin.compiler.plugin.ir.CallSiteValidator.computeReachableModules]'s BFS, using the same
     * cross-module includes-edge hint ([discoverModuleIncludesFromHints]) to walk past modules this
     * compilation cannot see the source of.
     *
     * A no-op when every included module is local (its own hints are already generated by
     * [generateDslDefinitionHints]) or when nothing relays.
     */
    fun relayIncludedDefinitionHints(
        moduleFragment: IrModuleFragment,
        moduleIncludes: Map<String, List<String>>,
        localModuleIds: Set<String>,
    ) {
        val farIncludedIds = mutableSetOf<String>()
        val queue = ArrayDeque(moduleIncludes.values.flatten().distinct())
        while (queue.isNotEmpty()) {
            val id = queue.removeFirst()
            if (id in localModuleIds) continue
            if (!farIncludedIds.add(id)) continue
            val edges = discoverModuleIncludesFromHints(id).edges
            for (edge in edges) if (edge !in farIncludedIds) queue.add(edge)
        }
        if (farIncludedIds.isEmpty()) return
        val hints = farIncludedIds.flatMap { relayDslDefinitionHints(it) }
        if (hints.isEmpty()) return

        val hintsPackage = KoinModuleFirGenerator.HINTS_PACKAGE
        val firModuleData = extractFirModuleDataFromModule(moduleFragment)
        if (firModuleData == null) {
            KoinPluginLogger.debug { "  WARN: relayIncludedDefinitionHints: no FIR module data, skipping ${hints.size} hint(s)" }
            return
        }
        val basePath = moduleFragment.files.minByOrNull { it.fileEntry.name }?.fileEntry?.name ?: "/synthetic"
        val fileName = HintFileNaming.fileName("koin_dsl_hints_relay_", KoinPluginLogger.moduleId)
        val fakeNewPath = Path(basePath).parent.resolve(fileName)
        ...
        KoinPluginLogger.debug {
            "  Relayed ${hints.size} DSL definition hint(s) from classpath-invisible includes: $farIncludedIds"
        }
    }
}
