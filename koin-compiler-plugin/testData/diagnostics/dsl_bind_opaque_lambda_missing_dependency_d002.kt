...
// still opaque, but the hand-written `get<Missing>()` call inside it is an ordinary IrCall the
// plugin never generated, so GeneratedResolutionCallRegistry doesn't skip it: it's tracked as a
// real call site like any other, and Missing having no provider is a hard KOIN-D002.
//
// Before this fix, this exact shape compiled clean (KOIN-W007, since removed, only disclosed it
// as unvalidated) and crashed at runtime with NoDefinitionFoundException the first time
// StubAnalyticsHelper was resolved — the worst failure class per this project's doctrine
// (silent > broken).
package testpkg
